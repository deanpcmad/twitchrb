  ...
  class Automod << Object
    ...
    class << self
      ...
      def check_status(broadcaster_id:, id:, text:)
        ...
        response = Client.post_request("moderation/enforcements/status", body: attrs)
      ...
      def manage_message(user_id:, msg_id:, action:)
        ...
        Client.post_request("moderation/automod/message", body: attrs)
      end

      def settings(broadcaster_id:, moderator_id:)
        response = Client.get_request("moderation/automod/settings?broadcaster_id=#{broadcaster_id}&moderator_id=#{moderator_id}")
      ...
      def update_settings(broadcaster_id:, moderator_id:, **params)
        ...
        response = Client.put_request("moderation/automod/settings", body: attrs.merge(params))
        AutomodSetting.new response.body.dig("data")[0]
      end
